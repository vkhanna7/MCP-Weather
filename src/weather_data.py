...
#
# A complete in-memory weather store. No API key needed — this file is
# provided as-is so you can focus on the MCP protocol, not the data layer.
#
# In a real app you would replace get_mock_weather / get_mock_forecast with
# calls to OpenWeatherMap, WeatherAPI, or similar.
...
def get_mock_weather(city: str) -> dict | None:
    """Return mock weather for *city*, or None if not found (case-insensitive)."""
    return WEATHER_DB.get(city.lower().strip())


def get_mock_forecast(city: str, days: int) -> list[dict] | None:
    """Return the first *days* daily forecasts for *city*, or None if not found."""
